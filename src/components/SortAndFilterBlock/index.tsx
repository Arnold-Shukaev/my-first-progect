...
export function sort (table: any[], {columnForSorting, sortingType}: StateSortType) {
    ...
    switch (sortingType) {
        case 'none':
            ...
            break;
        case 'asc':
            table.sort( (a: any, b: any) => a[columnSorting] > b[columnSorting] ? 1 : -1 );
            break;
        case 'desc':
            table.sort( (a: any, b: any) =>  b[columnSorting] > a[columnSorting] ? 1 : -1 );
            break;
        default: {
...
export function filter(table: any[], filteringRules: StateFilterType) {
    let countRules = 0;
    for (let rule in filteringRules) {countRules++}
    if (table.length === 0 || countRules === 0) return {resultListThings: table};

    let resultListThings = table;
    for (let rule in filteringRules) {
        let nameColumn = rule;
        let substring = filteringRules[rule].toLowerCase();
        resultListThings = resultListThings.filter((thing) => thing[nameColumn!].toString().toLowerCase().includes(substring))
    }

    const noFilteringResult = resultListThings.length === 0 ?
        <tr><td colSpan={table.length} style={{textAlign: 'center'}}>
...
({nameButton, nameParam, stateSort, setStateSort, setStateFilter}: Props) => {
