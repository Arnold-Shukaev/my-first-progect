...
function checkingElemHeightMoreParent(elem: HTMLDivElement): boolean {
    if(elem.parentNode === null) {
        console.log('Неверное применение checkingElemHeightMoreParent() в компоненте InputWithActivatingState. Нет родительского узла.')
        return false;
    }
    return (parseInt( getComputedStyle(elem).height) > parseInt( getComputedStyle(elem.parentNode as HTMLElement).height))
}
function shortenText(elem: HTMLDivElement): void {
    if(elem.parentNode === null) {
        console.log('Неверное применение shortenText() в компоненте InputWithActivatingState. Нет родительского узла.')
        return;
    }
...
export const InputWithActivatingState = ({label, propertyName, idThing, activeProperty, setActiveProperty, listThings, dispatchListThings}: Props) => {
    ...
    const [state, setState] = useState({h: document.documentElement.clientHeight, w: document.documentElement.clientWidth})
    useEffect( () => {
        function listener() {
            setState({h: document.documentElement.clientHeight, w: document.documentElement.clientWidth})
        }
    ...
    useEffect( () => { 
        ...
        if ( checkingElemHeightMoreParent(revText.current) ) {shortenText(revText.current)};
    ...
    return (
        <>
            <label htmlFor={propertyName + 'InputWithActivatingState'}><h4>{label}</h4></label>
            <div
                className={s.textProperty}
                ref={refAllBlock}
                id={propertyName + 'InputWithActivatingState'}
                onClick={ () => { if(!checkSelectThis) setActiveProperty(refAllBlock.current!.parentNode as HTMLDivElement)}}
                >
                ...
                <div
                    ...
                    <textarea
                        ...
                        value={textBeginEdited ? editableValue : listThings[idThing][propertyName] || ''}
                    />

                </div>
                <div className={s.moreText}></div>
                { checkSelectThis && <ButtonsForSpecialProperty
                    idThing={idThing}
