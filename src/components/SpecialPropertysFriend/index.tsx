...
const listLabel: string[] = ['Что любит', 'Что не любит', 'Назначенные встречи'];
const listPropertyName: string[] = ['Like', 'NotLike', 'Meetings'];

export const SpecialPropertyFriend = ({idThing, activeProperty, setActiveProperty, listThings, dispatchListThings}: Props) => {
    useEffect( () => {
    ...
    return (
        <div className={s.additionally}>
            ...
            {listLabel.map( (label, id) => {
                ...
                if (elem.props['data-special-property-name'] === activeProperty.dataset.specialPropertyName) return true;
                return false;
            })}
