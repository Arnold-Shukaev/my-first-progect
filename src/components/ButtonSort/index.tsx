...
export const ButtonSort = ({nameParam, stateSort, setStateSort}: Props) => {
    ...
    function hendleClickButton() {
        if (columnForSorting !== refButton.current) {
            ...
            sortingType === 'none' ? newSortingType = 'asc' :
            sortingType === 'asc' ? newSortingType = 'desc' :
            newSortingType = 'none'

            setStateSort({columnForSorting: refButton.current, sortingType: newSortingType})
        }
    }

    useEffect(() => {
        if (columnForSorting === refButton.current) {
            sortingType === 'none' ? refButton.current!.innerHTML = '' :
            sortingType === 'asc' ? refButton.current!.innerHTML = '&#8659' :
