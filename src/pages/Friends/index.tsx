...
const reduser: Reducer<StateType, ActionType> = (state, action): StateType => {
    ...
    switch (type) {
        case 'new':
            ...
            try {newList = JSON.parse(localStorage.specialStorage)}
            catch {
        ...
        case 'remove':
            ...
            const newSpecialStorage = interactionLocalStorage( (oldStorage, ...arg) => {
                delete oldStorage[arg[0][0]]
            }, action.idThing)
            return {listFriends: newSpecialStorage}
        case 'update':
            const newSpecialStorageAdd = interactionLocalStorage( (oldStorage, ...arg) => {
                oldStorage[arg[0][0]][arg[0][1]] = arg[0][2]
            }, action.idThing, action.nameProperty, action.newValue)
...
const columnsForBasicInfo = ['age', 'friends', 'score', 'fromCity', 'placeWork', 'email', 'phone'];
const columnsNameForBasicInfo = ['Возраст', 'Друзья', 'Оценка', 'Проживает в', 'Место работы', 'email', 'Телефон'];
...
export const Friends = (): JSX.Element => {
    ...
    const [activateSpesialProperty, setSpesialProperty] = useState<HTMLDivElement | null>(null);

    const [specialIdSelectedThing, setSpecialIdSelectedThings] = useState<string | null>(null);
    const selectedThing = (specialIdSelectedThing === null) ? null : listFriends[specialIdSelectedThing] || null;
    useEffect( () => {
        dispatch({type: 'new'})
    }, [])
    ...
    return (
        <div className={s.maket}>
            <div className={activateSpesialProperty ? s.activateSpesialProperty : undefined}>
                <DisplayListThings
